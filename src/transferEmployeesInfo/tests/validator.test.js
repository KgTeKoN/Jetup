...
describe('validate transfer schema', () => {
  test('validate correct data', async () => {
    const obj1 = {
      name: 'Andriy',
      position: 'Developer',
      essay: 'I am interesting in back-end',
    };
  ...
  test('try invalid position (is not be empty)', async () => {
    ...
    await expect(employeesSchema.validateAsync(obj2)).rejects.toThrow(
      '"position" is not allowed to be empty',
    );
  });
  ...
  test('try invalid position (must be a string)', async () => {
    ...
    await expect(employeesSchema.validateAsync(obj3)).rejects.toThrow(
  ...
  test('try invalid name (must be a string)', async () => {
    ...
    await expect(employeesSchema.validateAsync(obj4)).rejects.toThrow(
  ...
  test('try invalid name (must only contain alpha-numeric characters)', async () => {
    ...
    await expect(employeesSchema.validateAsync(obj5)).rejects.toThrow(
  ...
  test('try invalid name (is not be empty)', async () => {
    ...
    await expect(employeesSchema.validateAsync(obj6)).rejects.toThrow(
  ...
  test('try invalid essay (must be a string)', async () => {
    ...
    await expect(employeesSchema.validateAsync(obj7)).rejects.toThrow(
  ...
  test('try invalid essay (is not be empty)', async () => {
    ...
    await expect(employeesSchema.validateAsync(obj8)).rejects.toThrow(
      '"essay" is not allowed to be empty',
    );
  });
});
